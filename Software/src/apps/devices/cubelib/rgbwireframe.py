...
class Node:
    def __init__(self, coordinates,visible = False):
...
class Wireframe:
    ...
    def setVisible(self,visibleNodes):
    ...
    def outputNodes(self):
    	print "\n --- Nodes --- "
        for i, node in enumerate(self.nodes):
            print " %d: (%.2f, %.2f, %.2f)" % (i, node.x, node.y, node.z) , node.visiblity
...
if __name__ == "__main__":
    ...
    cube.outputNodes()
    cube.setVisible((0,1,3))    
    cube.addNodes([(100,100,100)])
    cube.outputNodes()
    cube.outputEdges()
