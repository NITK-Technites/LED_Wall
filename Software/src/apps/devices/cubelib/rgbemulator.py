...
#TODO:
# Convert to RGB emulator. 

import rgbwireframe as wireframe
import pygame
...
class ProjectionViewer:
    ...
    def __init__(self, width, height):
        ...
        pygame.display.set_caption('RGB Cube Simulator')
        ...
        for wireframe in self.wireframes.values():
            ...
            if self.displayNodes:
                for node in wireframe.nodes:
                    if node.visiblity:
                           pygame.draw.circle(self.screen, node.colour, (int(node.x), int(node.y)), self.nodeRadius, 0)
    ...
    def createCube(self,cube,X=[50,140], Y=[50,140], Z=[50,140]):
    	...
    	for i in range(0,4):
        	for j in range(0,4):
                	for k in range(0,4):
                        	allnodes.append((X[0]+(X[1]-X[0])/3 * i,Y[0]+(Y[1] - Y[0])/3 * j,Z[0] + (Z[1]-Z[0])/3 * k))
...
if __name__ == '__main__':
    ...
    sarray = numpy.array([[[[1]*3]*4]*4]*4)
    for x in range(0,4):
        for y in range(0,4):
            sarray[0][x][y] = [1,0,0]
    cube.setVisible(findIndexArray(sarray))
    count = 0 
    while(True):
	pv.run()
